...
func TestKeyEncoding(t *testing.T) {
	for i := 0; i < 1; i++ {
		...
		privateKey := hex.EncodeToHex(buf)
		t.Log("privateKey: " + privateKey)
		decodeBuf, err := hex.DecodeHex(privateKey)
		assert.NoError(t, err)

		priv0, err := ParseECDSAPrivateKey(decodeBuf)
		assert.NoError(t, err)
		...
		publickKey := hex.EncodeToHex(buf)
		t.Log("publickKey: " + publickKey)
		decodePubBuf, err := hex.DecodeHex(publickKey)
		assert.NoError(t, err)

		pub0, err := ParsePublicKey(decodePubBuf)
		assert.NoError(t, err)

		assert.Equal(t, priv.PublicKey, *pub0)

		address := PubKeyToAddress(pub0)
...
func TestPrivateKeyGeneration(t *testing.T) {
	...
	d := *readKey.D
	t.Log("key: " + hex.EncodeToString(d.Bytes()))
	t.Log("address: " + readAddress.String())
	assert.True(t, writtenKey.Equal(readKey))
	assert.Equal(t, writtenAddress.String(), readAddress.String())
}

func TestEncrpt(t *testing.T) {
	secretPass := "abc&1*~#^2^#s0^=)^^7%b34"

	StringToEncrypt := "Encrypting this string, hello!"

	encText, err := CFBEncrypt(StringToEncrypt, secretPass)
	if err != nil {
