...
type edgeSubscribeRegister struct{}

func (c *edgeSubscribeRegister) gas(input []byte, _ *chain.ForksInTime) uint64 {
	return 0
}

func (c *edgeSubscribeRegister) run(input []byte, caller types.Address, host runtime.Host) ([]byte, error) {
	//if len(input) < 1 {
