...
import (
	...
	txpoolOp "github.com/emc-protocol/edge-matrix/telepool/proto"
...
func generateTx(params GenerateTxReqParams) (*types.Telegram, error) {
	signer := crypto.NewEIP155Signer(chain.AllForksEnabled.At(0), 100)

	signedTx, signErr := signer.SignTele(&types.Telegram{
		Nonce:    params.Nonce,
