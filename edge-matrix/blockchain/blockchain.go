...
import (
	...
	"github.com/emc-protocol/edge-matrix/blockchain/storage"
	"github.com/emc-protocol/edge-matrix/blockchain/storage/leveldb"
	"github.com/emc-protocol/edge-matrix/blockchain/storage/memory"
...
func (b *Blockchain) readBody(hash types.Hash) (*types.Body, bool) {
	...
	if updated := b.recoverFromFieldsInTransactions(bb.Telegrams); updated {
...
func (b *Blockchain) verifyBlockBody(block *types.Block) ([]*types.Receipt, error) {
	...
	if hash := buildroot.CalculateTelegramsRoot(block.Telegrams); hash != block.Header.TeleRoot {
		b.logger.Error(fmt.Sprintf(
			"tele root hash mismatch: have %s, want %s",
			hash,
			block.Header.TeleRoot,
		))
...
func (br *BlockResult) verifyBlockResult(referenceBlock *types.Block) error {
	...
	if len(br.Receipts) != len(referenceBlock.Telegrams) {
...
func (b *Blockchain) updateGasPriceAvgWithBlock(block *types.Block) {
	if len(block.Telegrams) < 1 {
		// No transactions in the block,
		// so no gas price average to update
		return
	}

	gasPrices := make([]*big.Int, len(block.Telegrams))
	for i, transaction := range block.Telegrams {
...
func (b *Blockchain) writeBody(block *types.Block) error {
	...
	for _, txn := range block.Telegrams {
...
func (b *Blockchain) recoverFromFieldsInTransactions(transactions []*types.Telegram) bool {
	updated := false
...
func (b *Blockchain) GetBlockByHash(hash types.Hash, full bool) (*types.Block, bool) {
	...
	block.Telegrams = body.Telegrams
