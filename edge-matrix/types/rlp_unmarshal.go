...
type unmarshalRLPFromFunc func(TeleType, *fastrlp.Parser, *fastrlp.Value) error
...
func (t *TeleType) unmarshalRLPFrom(_ *fastrlp.Parser, v *fastrlp.Value) error {
	bytes, err := v.Bytes()
	if err != nil {
		return err
	}
...
func (b *Block) unmarshalRLPFrom(p *fastrlp.Parser, v *fastrlp.Value) error {
	...
	// transactions
	if err = unmarshalRLPFrom(p, elems[1], func(txType TeleType, p *fastrlp.Parser, v *fastrlp.Value) error {
		bTxn := &Telegram{
			Type: txType,
		}
		...
		b.Telegrams = append(b.Telegrams, bTxn)
...
func (h *Header) unmarshalRLPFrom(_ *fastrlp.Parser, v *fastrlp.Value) error {
	...
	if len(elems) < 12 {
		return fmt.Errorf("incorrect number of elements to decode header, expected 12 but found %d", len(elems))
	}
	...
	//}
	// miner
	if h.Miner, err = elems[1].GetBytes(h.Miner[:]); err != nil {
		return err
	}
	// stateroot
	...
	// teleroot
	if err = elems[3].GetHash(h.TeleRoot[:]); err != nil {
		return err
	}
	// receiptroot
	if err = elems[4].GetHash(h.ReceiptsRoot[:]); err != nil {
		return err
	}
	// logsBloom
	if _, err = elems[5].GetBytes(h.LogsBloom[:0], 256); err != nil {
		return err
	}
	// difficulty
	...
	//}
	// number
	if h.Number, err = elems[6].GetUint64(); err != nil {
		return err
	}
	// gasLimit
	if h.GasLimit, err = elems[7].GetUint64(); err != nil {
		return err
	}
	// gasused
	if h.GasUsed, err = elems[8].GetUint64(); err != nil {
		return err
	}
	// timestamp
	if h.Timestamp, err = elems[9].GetUint64(); err != nil {
		return err
	}
	// extraData
	if h.ExtraData, err = elems[10].GetBytes(h.ExtraData[:0]); err != nil {
		return err
	}
	// mixHash
...
func (r *Receipts) unmarshalRLPFrom(p *fastrlp.Parser, v *fastrlp.Value) error {
	return unmarshalRLPFrom(p, v, func(txType TeleType, p *fastrlp.Parser, v *fastrlp.Value) error {
		obj := &Receipt{
			TelegramType: txType,
		}
...
func (r *Receipt) UnmarshalRLP(input []byte) error {
	r.TelegramType = LegacyTx
	offset := 0

	if len(input) > 0 && input[0] <= RLPSingleByteUpperLimit {
		var err error
		if r.TelegramType, err = txTypeFromByte(input[0]); err != nil {
			return err
		}
