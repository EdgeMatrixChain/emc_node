...
func (q *accountQueue) clear() (removed []*types.Telegram) {
	// store txs
	removed = q.queue
...
func (q *accountQueue) pop() *types.Telegram {
	if q.length() == 0 {
		return nil
	}

	transaction, ok := heap.Pop(&q.queue).(*types.Telegram)
	if !ok {
		return nil
	}
...
type minNonceQueue []*types.Telegram

/* Queue methods required by the heap interface */

func (q *minNonceQueue) Peek() *types.Telegram {
	if q.Len() == 0 {
		return nil
	}
...
func (q *minNonceQueue) Push(x interface{}) {
	transaction, ok := x.(*types.Telegram)
	if !ok {
		return
	}
...
func (q *pricedQueue) push(tx *types.Telegram) {
	heap.Push(&q.queue, tx)
}

// Pop removes the first transaction from the queue
// or nil if the queue is empty.
func (q *pricedQueue) pop() *types.Telegram {
	if q.length() == 0 {
		return nil
	}
...
type maxPriceQueue []*types.Telegram

/* Queue methods required by the heap interface */

func (q *maxPriceQueue) Peek() *types.Telegram {
	if q.Len() == 0 {
		return nil
	}
