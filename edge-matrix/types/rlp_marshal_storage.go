...
func (b *Body) marshalRLPWith(ar *fastrlp.Arena) *fastrlp.Value {
	vv := ar.NewArray()
	if len(b.Telegrams) == 0 {
		vv.Set(ar.NewNullArray())
	} else {
		v0 := ar.NewArray()
		for _, tx := range b.Telegrams {
...
func (t *Telegram) MarshalStoreRLPTo(dst []byte) []byte {
	return MarshalRLPTo(t.marshalStoreRLPWith, dst)
}

func (t *Telegram) marshalStoreRLPWith(a *fastrlp.Arena) *fastrlp.Value {
	vv := a.NewArray()
...
func (r *Receipt) marshalStoreRLPWith(a *fastrlp.Arena) *fastrlp.Value {
	...
	if r.ApplicationAddress == nil {
		vv.Set(a.NewNull())
	} else {
		vv.Set(a.NewBytes(r.ApplicationAddress.Bytes()))
	}
