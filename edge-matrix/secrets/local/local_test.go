...
func getLocalSecretsManager(t *testing.T) secrets.SecretsManager {
	...
	setupErr := common.SetupDataDir(workingDirectory, []string{secrets.ConsensusFolderLocal, secrets.NetworkFolderLocal, secrets.ICPFolderLocal}, 0770)
	if setupErr != nil {
...
func TestLocalSecretsManager_GetSetSecret(
	...
	ed25519PrivKey, _, genErr := crypto.GenerateAndEncodeICPIdentitySecretKey()
	if genErr != nil {
		t.Fatalf("Unable to generate ICP identity private key, %v", genErr)
	}
	...
	// Compare ICP identity keys helper
	compareICPKeys := func(manager secrets.SecretsManager) bool {
		parsedKey, parseErr := crypto.ReadICPIdentityKey(manager)
		if parseErr != nil {
			t.Fatalf("unable to parse validator private key, %v", parseErr)
		}
		t.Log(hex.EncodeToString(ed25519PrivKey))
		return ed25519PrivKey.Equal(parsedKey)
	}
