...
// takeBlockUpdates advances blocks from head to latest and returns msg array
...
func NewFilterManager(logger hclog.Logger, store filterManagerStore, blockRangeLimit uint64) *FilterManager {
	...
	// start blockstream with the current msg
	header := store.Header()
		...
		for logIdx, log := range receipt.Logs {
			if query.Match(log) {
				logs = append(logs, &Log{
					...
					TxHash:      block.Telegrams[idx].Hash,
...
func (f *FilterManager) appendLogsToFilters(header *block) error {
	...
	for indx, receipt := range receipts {
		if receipt.TxHash == types.ZeroHash {
			// Extract tx Hash
			receipt.TxHash = block.Telegrams[indx].Hash
		}
