...
import (
	...
	"github.com/emc-protocol/edge-matrix/state/runtime/evm"
	"github.com/emc-protocol/edge-matrix/state/runtime/precompiled"
	"github.com/emc-protocol/edge-matrix/state/runtime/tracer"
...
func (t *Transition) WriteFailedReceipt(txn *types.Telegram) error {
	...
	if txn.To == nil {
		receipt.ApplicationAddress = crypto.CreateAddress(txn.From, txn.Nonce).Ptr()
	}
...
func (t *Transition) Write(tele *types.Telegram) error {
	var err error

	if tele.From == emptyFrom && tele.Type == types.LegacyTx {
		// Decrypt the from address
		signer := crypto.NewSigner(t.config, uint64(t.ctx.ChainID))

		tele.From, err = signer.Sender(tele)
		if err != nil {
			return NewTransitionApplicationError(err, false)
		}
	}
	...
	result, e := t.Apply(teleCopy)
	if e != nil {
		t.logger.Error("failed to apply telegram", "err", e)
	...
	if tele.To == nil {
		receipt.ApplicationAddress = crypto.CreateAddress(tele.From, tele.Nonce).Ptr()
	}
...
func (t *Transition) Apply(tele *types.Telegram) (*runtime.ExecutionResult, error) {
	s := t.state.Snapshot()

	result, err := t.apply(tele)
	if err != nil {
...
func (t *Transition) subGasLimitPrice(msg *types.Telegram) error {
...
func (t *Transition) nonceCheck(msg *types.Telegram) error {
...
// modified by shawn
func (t *Transition) apply(tele *types.Telegram) (*runtime.ExecutionResult, error) {
	if tele.Type == types.StateTx {
		if err := checkAndProcessStateTx(tele, t); err != nil {
			return nil, err
		}
	} else {
		if err := checkAndProcessLegacyTx(tele, t); err != nil {
			return nil, err
		}
	}
	...
	//	for gas free
	intrinsicGasCost := uint64(0)

	// the purchased gas is enough to cover intrinsic usage
	gasLeft := tele.Gas - intrinsicGasCost
	...
	gasPrice := new(big.Int).Set(tele.GasPrice)
	value := new(big.Int).Set(tele.Value)

	// set the specific transaction fields in the context
	t.ctx.GasPrice = types.BytesToHash(gasPrice.Bytes())
	t.ctx.Origin = tele.From

	var result *runtime.ExecutionResult
	if tele.IsContractCreation() {
		result = t.Create2(tele.From, tele.Input, value, gasLeft)
	} else {
		t.state.IncrNonce(tele.From)
		result = t.Call2(tele.From, *tele.To, tele.Input, value, gasLeft)
	}

	refund := t.state.GetRefund()
	result.UpdateGasUsed(tele.Gas, refund)
	...
	// refund the sender
	remaining := new(big.Int).Mul(new(big.Int).SetUint64(result.GasLeft), tele.GasPrice)
	t.state.AddBalance(tele.From, remaining)

	// pay the coinbase
	coinbaseFee := new(big.Int).Mul(new(big.Int).SetUint64(result.GasUsed), tele.GasPrice)
...
func TransactionGasCost(msg *types.Telegram, isHomestead, isIstanbul bool) (uint64, error) {
	cost := uint64(0)
...
func checkAndProcessLegacyTx(msg *types.Telegram, t *Transition) error {
	// 1. the nonce of the message caller is correct
...
func checkAndProcessStateTx(msg *types.Telegram, t *Transition) error {
