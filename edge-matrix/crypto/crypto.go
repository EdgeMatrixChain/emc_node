...
import (
	...
	"github.com/emc-protocol/edge-matrix/helper/keystore"
...
var (
	...
	zero                   = big.NewInt(0)
	one                    = big.NewInt(1)
	iv                     = []byte{30, 36, 57, 24, 25, 35, 24, 74, 87, 35, 18, 98, 66, 32, 14, 05}
...
func BytesToECDSAPrivateKey(input []byte) (*ecdsa.PrivateKey, error) {
	// The key file on disk should be encoded in hexString,
...
func BytesToEd25519PrivateKey(input []byte) (ed25519.PrivateKey, error) {
	...
	// Make sure the key is properly formatted
	if len(decoded) != 64 {
		// Key must be exactly 128 chars (64B) long
		return nil, fmt.Errorf("invalid ed25519 key length (%dB), should be 64B", len(decoded))
	}
