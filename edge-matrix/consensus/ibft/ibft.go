...
import (
	...
	"github.com/emc-protocol/edge-matrix/consensus/ibft/proto"
	...
	"github.com/emc-protocol/edge-matrix/syncer"
...
type txPoolInterface interface {
	...
	Length() uint64
	Peek() *types.Telegram
	Pop(tx *types.Telegram)
	Drop(tx *types.Telegram)
	Demote(tx *types.Telegram)
...
type backendIBFT struct {
	...
	telepool       txPoolInterface        // Reference to the transaction pool
...
func (i *backendIBFT) startSyncing() {
	callInsertBlockHook := func(fullBlock *types.FullBlock) bool {
		...
		i.telepool.ResetWithHeaders(fullBlock.Block.Header)
...
func (i *backendIBFT) startConsensus() {
	...
	for {
		...
		i.telepool.SetSealing(isValidator)
...
func (i *backendIBFT) updateMetrics(block *types.Block) {
	...
	metrics.SetGauge([]string{consensusMetrics, "num_txs"}, float32(len(block.Body().Telegrams)))
}
...
func (i *backendIBFT) GetSignerAddress() types.Address {
	return i.currentSigner.Address()
}
