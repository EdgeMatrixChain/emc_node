...
// The PocQueue is implemented as priority queue which utilizes a heap (standard Go implementation)
// https://golang.org/src/container/heap/heap.go
type dialQueueImpl []*PocTask
...
func (t *dialQueueImpl) Push(x interface{}) {
	n := len(*t)
	item := x.(*PocTask) //nolint:forcetypeassert
	item.index = n
	*t = append(*t, item)
}
