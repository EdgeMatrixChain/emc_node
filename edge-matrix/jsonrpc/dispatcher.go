...
func (d *Dispatcher) handleEdgeSubscribe(req Request, conn wsConn) (string, Error) {
	...
	if subscribeMethod == "rtc" {
		...
		decodeHex, err := hex.DecodeHex(params[1].(string))
		if err != nil {
			return "", NewInvalidRequestError(err.Error())
		}
		msg := &rtc.RtcMsg{}
		if err := msg.UnmarshalRLP(decodeHex); err != nil {
			return "", NewInvalidRequestError(err.Error())
		}

		// prepare to new RtcFilter
		rtcQuery := &RtcQuery{
			Subject:     msg.Subject,
			Application: msg.Application,
		}

		if len(rtcQuery.Subject) < 0 {
			return "", NewInvalidRequestError(fmt.Sprintf("failed to subscribe rtc subject: Subject string len=%d", len(rtcQuery.Subject)))
		}

		if len(rtcQuery.Application) < 0 {
			return "", NewInvalidRequestError(fmt.Sprintf("failed to subscribe rtc subject: Application string len=%d", len(rtcQuery.Application)))
		}

		hash := types.StringToHash(rtcQuery.Subject)

		// TODO should to call d.endpoints.Edge.GetTelegramReceipt(hash) and verify .To==contracts.EdgeRtcSubjectPrecompile
		obj, err := d.endpoints.Edge.GetTelegramByHash(hash)
		if err != nil {
			return "", NewInternalError(err.Error())
		}
		if obj == nil {
			return "", NewInvalidRequestError(fmt.Sprintf("failed to broadcast to subject: %s is not exist", rtcQuery.Subject))
		}

		rm := &RtcMsg{
			To:          msg.To.String(),
			Application: rtcQuery.Application,
			Subject:     rtcQuery.Subject,
			Content:     msg.Content,
			V:           msg.V.String(),
			R:           msg.R.String(),
			S:           msg.S.String(),
			Type:        rtc.SubscribeMsg,
		}
		sender, err := d.endpoints.Edge.Sender(rm)
		if err != nil {
			return "", nil
		}
		rtcQuery.From = sender
		// broadcast new subscriber to subject
		_, err = d.endpoints.Edge.SendMsg(rm)
		if err != nil {
