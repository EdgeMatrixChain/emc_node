...
func (m *accountsMap) getPrimaries() (primaries []*types.Telegram) {
...
func (m *accountsMap) allTxs(includeEnqueued bool) (
	...
	allPromoted = make(map[types.Address][]*types.Telegram)
	allEnqueued = make(map[types.Address][]*types.Telegram)
...
func (a *account) enqueue(tele *types.Telegram) error {
	a.enqueued.lock(true)
	defer a.enqueued.unlock()
...
// nextNonce.
func (a *account) promote() (promoted []*types.Telegram, pruned []*types.Telegram) {
	a.promoted.lock(true)
	a.enqueued.lock(true)
	...
	// move all promotable teles (enqueued teles that are sequential in nonce)
...
func (a *account) getLowestTx() *types.Telegram {
	a.promoted.lock(true)
	defer a.promoted.unlock()
