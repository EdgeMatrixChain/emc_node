...
func (b *Body) unmarshalRLPFrom(p *fastrlp.Parser, v *fastrlp.Value) error {
	...
	// transactions
	if err = unmarshalRLPFrom(p, tuple[0], func(txType TeleType, p *fastrlp.Parser, v *fastrlp.Value) error {
		bTxn := &Telegram{
			Type: txType,
		}
...
func (t *Telegram) UnmarshalStoreRLP(input []byte) error {
	t.Type = LegacyTx
...
func (t *Telegram) unmarshalStoreRLPFrom(p *fastrlp.Parser, v *fastrlp.Value) error {
...
func (r *Receipt) unmarshalStoreRLPFrom(p *fastrlp.Parser, v *fastrlp.Value) error {
	...
	// come TransactionType first if exist
	if len(elems) == 5 {
		if err = r.TelegramType.unmarshalRLPFrom(p, elems[0]); err != nil {
			return err
		}
	...
	if len(vv) == 20 {
		// address
		r.SetApplicationAddress(BytesToAddress(vv))
	}
