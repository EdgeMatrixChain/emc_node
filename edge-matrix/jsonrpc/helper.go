...
var (
	ErrHeaderNotFound           = errors.New("msg not found")
	ErrLatestNotFound           = errors.New("latest msg not found")
	ErrNegativeBlockNumber      = errors.New("invalid argument 0: block number must not be negative")
	ErrFailedFetchGenesis       = errors.New("error fetching genesis block msg")
...
// GetBlockHeader returns a msg using the provided number
func GetBlockHeader(number BlockNumber, store headerGetter) (*types.Header, error) {
	...
	default:
		...
		if !ok {
			return nil, fmt.Errorf("error fetching block number %d msg", uint64(number))
		}
...
func GetTxAndBlockByTxHash(txHash types.Hash, store txLookupAndBlockGetter) (*types.Telegram, *types.Block) {
...
func GetHeaderFromBlockNumberOrHash(bnh BlockNumberOrHash, store blockGetter) (*types.Header, error) {
	...
	if bnh.BlockNumber != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to get the msg of block %d: %w", *bnh.BlockNumber, err)
		}
...
func DecodeTxn(arg *txnArgs, store nonceGetter) (*types.Telegram, error) {
	// set default values
	if arg.From == nil {
