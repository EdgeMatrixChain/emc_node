...
type block struct {
	...
	TeleRoot     types.Hash  `json:"transactionsRoot"`
	...
	Hash      types.Hash       `json:"hash"`
	Telegrams []telegramOrHash `json:"telegrams"`
...
func toBlock(b *types.Block, fullTx bool) *block {
	...
	res := &block{
		...
		Nonce:     h.Nonce,
		Hash:      h.Hash,
		Telegrams: []telegramOrHash{},
		Uncles:    []types.Hash{},
	}

	for idx, txn := range b.Telegrams {
		if fullTx {
			res.Telegrams = append(
				res.Telegrams,
				toTransaction(
					txn,
					argUintPtr(b.Number()),
...
type receipt struct {
	...
	BlockNumber        argUint64      `json:"blockNumber"`
	GasUsed            argUint64      `json:"gasUsed"`
	ApplicationAddress *types.Address `json:"applicationAddress"`
