...
func TestEth_Block_GetBlockTransactionCountByNumber(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		block.Telegrams = append(block.Telegrams, []*types.Telegram{{Nonce: 0, From: addr0}}...)
	}
	store.add(block)

	eth := newTestEthEndpoint(store)

	res, err := eth.GetBlockTelegramCountByNumber(BlockNumber(block.Header.Number))
...
func TestEth_GetTransactionByHash(t *testing.T) {
	...
	t.Run("returns correct transaction data if transaction is found in a sealed block", func(t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			...
			block.Telegrams = append(block.Telegrams, txn)
		}

		testTxnIndex := 5
		testTxn := block.Telegrams[testTxnIndex]

		res, err := eth.GetTelegramByHash(testTxn.Hash)
		assert.NoError(t, err)
		assert.NotNil(t, res)
	...
	t.Run("returns nil if transaction is nowhere to be found", func(t *testing.T) {
		...
		res, err := eth.GetTelegramByHash(types.StringToHash("abcdef"))
...
func TestEth_GetTransactionReceipt(t *testing.T) {
	...
	t.Run("returns nil if transaction with same hash not found", func(t *testing.T) {
		...
		res, err := eth.GetTelegramReceipt(hash1)
	...
	t.Run("returns correct receipt data for found transaction", func(t *testing.T) {
		...
		res, err := eth.GetTelegramReceipt(txn.Hash)
...
func (m *mockBlockStore) GetPendingTx(txHash types.Hash) (*types.Telegram, bool) {
...
func (m *mockBlockStore) ApplyTxn(header *types.Header, txn *types.Telegram) (*runtime.ExecutionResult, error) {
